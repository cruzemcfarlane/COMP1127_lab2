...
def check_price(bookshop,isbn):
    for i in range(0,len(get_title(bookshop))):
        if isbn==(get_title(bookshop)[i]):
            return get_title(bookshop)[i][6]
    return 'Book not found'

def books_to_reorder(bookshop,qty):
    orderlist=[]
    for i in range(0,len(get_title(bookshop))):
        if get_title(bookshop)[i][5]<qty:
            orderlist+=[(get_title(bookshop)[i][0],get_title(bookshop)[i][1])]
    return orderlist
